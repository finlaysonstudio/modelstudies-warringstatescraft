...
- Run artifacts are JSON under `data/` (`runs/`, `interview/`, `probe/`, `scorecards/`); the app's dev server serves them at `/data/*`. The run schema lives in `packages/game/src/types.ts` and is mirrored in `packages/app/src/lib/types.ts` — change both together.
